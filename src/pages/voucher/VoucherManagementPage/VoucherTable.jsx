...
const VoucherTable = ({ data, isLoading, onEdit, onDelete, onDisable, onViewStats }) => {
    const { event } = useContext(EventContext);

    if (isLoading) return <div className="p-8 text-center text-muted-foreground">Đang tải danh sách voucher...</div>;
    if (!data || data.length === 0) return <div className="p-8 text-center text-muted-foreground">Chưa có voucher nào được tạo.</div>;
                        ...
                        <TableRow key={voucher.id || index} className="hover:bg-muted/50">
                            ...
                            <TableCell>
                                <VoucherStatusBadge status={voucher.status}
                                    start={voucher.startDateTime} end={voucher.endDateTime} />
                            </TableCell>
                            ...
                            <TableCell className="text-right pr-4">
                                <DropdownMenu>
                                    ...
                                    <DropdownMenuContent align="end">
                                        ...
                                        {/* disable button */}
                                        <DropdownMenuItem
                                            className="text-orange-600 focus:bg-orange-600/10 focus:text-orange-600 
                                            cursor-pointer font-medium"
                                            onClick={() => onDisable(voucher)}
                                        >
                                            {voucher.status == CouponStatus.ACTIVE ?
                                                'Vô hiệu hóa voucher' : 'Bật hoạt động voucher'}
                                        </DropdownMenuItem>
