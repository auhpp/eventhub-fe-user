...
import ImageUploadSession from '../../event/CreateEventPage/ImageUploadSession';
import EventBasicInfo from '../../event/CreateEventPage/EventBasicInfo';
import EventLocation from '../../event/CreateEventPage/EventLocation';
...
const updateEventSchema = z.object({
    ...
    tags: z.array(z.object({
        ...
        name: z.string()
    })).min(1, { message: "Vui lòng thêm ít nhất 1 thẻ (Tag) cho sự kiện" }),
    hasResalable: z.boolean().default(true),
...
const EditEventPage = () => {
    ...
    const fetchEventData = async () => {
        if (!id) return;
        try {
            const [eventResponse, tagsResponse] = await Promise.all([
                getEventById({ id }),
                getAllTags({ eventId: id })
            ]);

            if (eventResponse) {
                const eventDataRes = eventResponse.result;
                setEventData(eventDataRes);

                const rawTags = tagsResponse?.result || [];
                ...
                form.reset({
                    ...
                    hasResalable: eventDataRes.hasResalable ?? true,
                    tags: formattedTags
                });
            }
        } catch (error) {
            console.error("Failed to fetch event data:", error);
            toast.error("Không thể tải thông tin sự kiện hoặc thẻ (tags)");
