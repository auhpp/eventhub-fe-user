...
import { Ticket, Trash2, PlusCircle, CalendarClock, Link as LinkIcon, KeyRound, Video, ExternalLink, MessageSquareQuote } from 'lucide-react';
...
const EventSessions = ({ sessions, setSessions, eventType }) => {
    ...
    const validateSession = (session) => {
        ...
        if (start && start <= now) errors.startTime = "Thời gian bắt đầu phải lớn hơn hiện tại";
        if (start && end && end <= start) errors.endTime = "Thời gian kết thúc phải lớn hơn thời gian bắt đầu";
        if (checkin && start && checkin > start) errors.checkinStartTime = "Thời gian check-in phải trước khi bắt đầu";
            ...
            <CardHeader className="border-b bg-slate-50/50 rounded-t-2xl pb-4 flex flex-row items-center justify-between">
                <div className="space-y-1">
                    <CardTitle className="flex items-center gap-2 text-lg">
                        <span className="size-7 rounded-lg bg-blue-600 text-white flex items-center justify-center text-sm font-bold shadow-sm">
                                    ...
                                    <div className={cn("text-left flex gap-3 me-1 ", (session.startTime ? "items-start" : "items-center"))}>
                                        ...
                                        <div>
                                            <h4 className={cn("font-bold text-base text-slate-800",
                                                (!session.startTime || (eventType === 'ONLINE' && !session.meetingUrl)) && " text-red-500"
                                            )}>
                            ...
                            <AccordionContent className="pb-4 pt-2 space-y-6">
                                <div className="space-y-4">
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mx-1">
                                        <div>
                                            ...
                                            />
                                            <p className="text-xs text-muted-foreground mt-1 ml-1">Thời gian bắt đầu cho phép tham gia.</p>
                                        </div>
                                        <DateTimePicker
                                        ...
                                        <div className="bg-blue-50/50 p-4 rounded-xl border border-blue-100 space-y-4">
                                            <div className="flex items-center justify-between">
                                                ...
                                                <div className="flex gap-2">
                                                    <Button type="button" variant="outline" size="sm" className="h-7 text-xs bg-white text-slate-600 border-slate-200 hover:text-blue-600 hover:border-blue-300" onClick={() => window.open('https://meet.google.com/', '_blank')}>
                                                        Tạo Google Meet <ExternalLink className="size-3 ml-1 opacity-50" />
                                                    </Button>
                                                    <Button type="button" variant="outline" size="sm" className="h-7 text-xs bg-white text-slate-600 border-slate-200 hover:text-blue-600 hover:border-blue-300" onClick={() => window.open('https://zoom.us/meeting/schedule', '_blank')}>
                                ...
                                <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-4">
                                    <h5 className="font-bold text-sm flex items-center gap-2 text-slate-800">
                                        <MessageSquareQuote className="size-4 text-blue-600" /> Cài đặt Hỏi Đáp (Q&A)
                                    </h5>
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        <div className="space-y-2">
                                            <Label className="text-sm">Trạng thái Q&A</Label>
                                            <Select
                                                value={session.qaStatus || 'DISABLED'}
                                                onValueChange={(val) => handleSessionChange(session.id, 'qaStatus', val)}
                                            >
                                                <SelectTrigger className="bg-white">
                                                    <SelectValue placeholder="Chọn trạng thái" />
                                                </SelectTrigger>
                                                <SelectContent>
                                                    <SelectItem value="DISABLED">Tắt Q&A</SelectItem>
                                                    <SelectItem value="PRE_EVENT">Nhận câu hỏi trước (Pre-event)</SelectItem>
                                                    <SelectItem value="LIVE">Trực tiếp (Live)</SelectItem>
                                    ...
                                    {(session.qaStatus === 'PRE_EVENT' || session.qaStatus === 'LIVE') && (
                                        <div className="flex flex-col sm:flex-row gap-6 mt-4 p-4 rounded-xl border bg-white shadow-sm">
                                            <div className="flex items-center space-x-3">
                                                <Switch
                                                    id={`anonymous-${session.id}`}
                                                    checked={session.allowAnonymous || false}
                                                    onCheckedChange={(val) => handleSessionChange(session.id, 'allowAnonymous', val)}
                                                />
                                            ...
                                            <div className="flex items-center space-x-3">
                                                <Switch
                                                    ...
                                                    checked={session.requireModerationQuestion || false}
                                                    onCheckedChange={(val) => handleSessionChange(session.id, 'requireModerationQuestion', val)}
                                                />
