...
import { AttendeeType, BookingType } from '@/utils/constant';
...
const OrderDetailPage = () => {
    ...
    useEffect(() => {
        const fetchBookingById = async () => {
            try {
                const response = await getBookingById({ id: orderId })
    ...
    const handleExport = async () => {
        try {
            setIsExporting(true);
            var invoiceUrl;
            if (booking.invoiceUrl == null || booking.invoiceUrl == "") {
                const res = await getInvoiceUrl({ bookingId: orderId })
                invoiceUrl = res.result
                setBooking({ ...booking, invoiceUrl: invoiceUrl })
                window.open(invoiceUrl, "_target")
            }
            else {
                invoiceUrl = booking.invoiceUrl;
                window.open(invoiceUrl, "_target")
            }
            // invoiceUrl = invoiceUrl.replace('/upload/', `/upload/fl_attachment:hoa-don-${orderId}/`);
            // const link = document.createElement('a');
            // link.href = invoiceUrl;
            // link.setAttribute('download', `hoa-don-${orderId}.pdf`); // Thuộc tính download của HTML5
            ...
            <div className="space-y-4">
                ...
                <Card className="shadow-sm border-slate-200">
                    <CardHeader className="pb-2 border-b border-slate-100">
                        <CardTitle className="text-sm font-bold text-slate-900 uppercase tracking-wider flex items-center gap-2">
                            {booking.type == AttendeeType.BUY ? 'Thông tin người mua' :
                                'Thông tin khách mời'}
                        </CardTitle>
                    </CardHeader>
                ...
                <Card className="shadow-sm border-slate-200 overflow-hidden">
                    <CardHeader className="pb-4 border-b border-slate-100 bg-slate-50/50">
                        <CardTitle className="text-sm font-bold text-slate-900 uppercase tracking-wider flex items-center gap-2">
                            Danh sách vé ({booking.attendees?.length || 0})
                        </CardTitle>
                    </CardHeader>
                    ...
                    <Card className="shadow-sm border-slate-200 flex flex-col h-full">
                        ...
                        <CardContent className="space-y-5 pt-4 flex-1">
                            <div className="flex justify-between items-center py-2 border-b border-dashed border-slate-200">
                                ...
                                {
                                    booking.type != BookingType.INVITE &&
