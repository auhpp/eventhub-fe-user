...
import PendingBookingModal from '@/pages/payment/TicketSelectionPage/PendingBookingModal';
...
const ResaleTicketSelectionPage = () => {
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            ...
            try {
                ...
                // init quantity
                const availableCount = postData?.attendees?.filter(a => a.status === AttendeeStatus.ON_RESALE.key).length || 0;
                if (availableCount > 0) {
    ...
    const onSubmitCreatePendingBooking = async () => {
        ...
        try {
            ...
            // Handle catch MAXIMUM_TICKET_PER_USER
    ...
    useEffect(() => {
        if (ticketInfo?.id && user) {
            const fetchBoughtQuantity = async () => {
                try {
                    const response = await countBoughtTicket({ ticketId: ticketInfo.id, userId: user.id });
                    setBoughtQuantity(response?.result ?? response ?? 0);
                } catch (error) {
                    console.error("Lỗi lấy số lượng vé đã mua:", error);
                    setBoughtQuantity(0);
                }
            };
            fetchBoughtQuantity();
    ...
    const remainingQuota = ticketInfo?.maximumPerUser != null
        ? Math.max(0, ticketInfo.maximumPerUser - boughtQuantity)
        : maxQuantity;

    const actualMaxAllowed = Math.min(maxQuantity, remainingQuota);

    const isBlockedFromCombo = !resalePost?.hasRetail && maxQuantity > remainingQuota;

    useEffect(() => {
        if (resalePost?.hasRetail && quantity > actualMaxAllowed && actualMaxAllowed > 0) {
            setQuantity(actualMaxAllowed);
        }
    }, [actualMaxAllowed, resalePost, quantity]);
    ...
    const handleQuantityChange = (delta) => {
        ...
        setQuantity(prev => {
            ...
            if (newQty > actualMaxAllowed) {
                if (delta > 0 && actualMaxAllowed === remainingQuota && ticketInfo?.maximumPerUser != null) {
                    toast.warning(`Bạn chỉ có thể mua thêm tối đa ${remainingQuota} vé (Đã mua: ${boughtQuantity}/${ticketInfo.maximumPerUser}).`);
                }
                return prev;
            }
            return newQty;
                                        ...
                                        sm:justify-center gap-3 pt-4 sm:pt-0 border-t sm:border-t-0 border-gray-200">
                                            ...
                                            <div className={`flex items-center gap-3 rounded-lg border p-1 bg-white 
                                                ...
                                                <Button
                                                    ...
                                                    onClick={() => handleQuantityChange(1)}
                                                    disabled={!resalePost.hasRetail || quantity >= actualMaxAllowed}                                                >
                    ...
                    <div className="w-full lg:w-[380px] shrink-0">
                        <div className="sticky top-4">
                            <BookingSummary
                                ...
                                messageButton={
                                    isBlockedFromCombo ? "Vượt giới hạn vé/người"
                                        : remainingQuota === 0 ? "Bạn đã hết lượt mua"
                                            : isSubmitting ? "Đang xử lý..."
                                                : "Tiến hành thanh toán"
                                }
                                disabled={maxQuantity === 0 || isSubmitting || remainingQuota === 0 || isBlockedFromCombo}
                            />
                        </div>
                    </div>
                </div>
