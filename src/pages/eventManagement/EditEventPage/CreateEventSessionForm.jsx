...
import { PlusCircle, X, Video, ExternalLink, Loader2, MessageSquareQuote } from 'lucide-react';
...
const CreateEventSessionForm = ({
    ...
    return (
        <div className="border-2 border-blue-100 bg-blue-50/30 rounded-xl p-4 space-y-5 animate-in fade-in slide-in-from-bottom-4">
                ...
                <div className="bg-white p-4 rounded-xl border space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <Input placeholder="123456" value={newSessionData.meetingPassword} onChange={(e) => onChange('meetingPassword', e.target.value)} />
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="bg-white p-4 rounded-xl border space-y-4">
                <h5 className="font-bold text-sm text-blue-700 flex items-center gap-2">
                    <MessageSquareQuote className="size-4" /> Cài đặt Hỏi Đáp (Q&A)
                </h5>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="space-y-2">
                        <Label className="text-sm">Trạng thái Q&A</Label>
                        <Select
                            value={newSessionData.qaStatus}
                            onValueChange={(val) => onChange('qaStatus', val)}
                        >
                            <SelectTrigger>
                ...
                {(newSessionData.qaStatus === 'PRE_EVENT' || newSessionData.qaStatus === 'LIVE') && (
                    <div className="flex flex-col sm:flex-row gap-6 mt-2">
                        <div className="flex items-center space-x-2">
                            <Switch
                                id={`new-anonymous`}
                                checked={newSessionData.allowAnonymous}
                                onCheckedChange={(val) => onChange('allowAnonymous', val)}
                            />
                            <Label htmlFor={`new-anonymous`} className="text-sm font-normal cursor-pointer">
                        ...
                        <div className="flex items-center space-x-2">
                            <Switch
                                id={`new-moderation`}
                                checked={newSessionData.requireModerationQuestion}
                                onCheckedChange={(val) => onChange('requireModerationQuestion', val)}
                            />
                            <Label htmlFor={`new-moderation`} className="text-sm font-normal cursor-pointer">
            ...
            <div className="bg-white p-4 rounded-lg border border-slate-200">
                <div className="flex justify-between items-center mb-3">
                    ...
                    <Button size="sm" variant="outline" onClick={() => onOpenCreateTicket(newSessionData, true)} className="text-xs h-8">
