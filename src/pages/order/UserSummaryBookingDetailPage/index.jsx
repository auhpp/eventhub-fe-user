...
import { Loader2, Mail, Phone, CheckCircle, ArrowLeft } from "lucide-react";
            ...
            bookings.forEach(booking => {
                booking.attendees.forEach(attendee => {
                    if (attendee.owner?.email === user.email) {
                        totalTicketsTmp++;
                        if (attendee.status === AttendeeStatus.CHECKED_IN?.key || attendee.status === AttendeeStatus.OUTSIDE.key) {
                            checkInTmp++;
                        }
                    }
                });
                    ...
                    <div className="grid grid-cols-1 gap-4">
                        {bookings.map((booking) => (
                            <AttendeeBookingCard
                                key={booking.id}
                                booking={booking}
                                onCheckInClick={handleOpenConfirmDialog} 
