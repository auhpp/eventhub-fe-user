...
import { AttendeeStatus, BookingType, EventType, QAStatus } from '@/utils/constant';
...
const TicketDetailPage = () => {
    ...
    const isBuyer = booking?.appUser?.id === user?.id;

    const displayAttendees = isBuyer
        ? booking?.attendees
        : booking?.attendees?.filter(att => att.owner?.id === user?.id);

    const attendeeForReview = booking?.attendees?.find(att =>
        (att.status === 'CHECKED_IN' || att.status === AttendeeStatus.OUTSIDE.key) && att.owner?.id === user?.id
    );
    const attendeeIdForReview = attendeeForReview?.id;
    ...
    const canReview = isEventExpired && Boolean(attendeeIdForReview);
    const isInvited = booking.type === BookingType.INVITE;
    const canAccessQA = liveQaStatus === QAStatus.PRE_EVENT || liveQaStatus === QAStatus.LIVE;
            ...
            <main className="container mx-auto">
                ...
                <div className="mb-8">
                    <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-6 gap-4">
                        <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
                            ...
                            Vé của bạn ({displayAttendees?.length || 0})
                        </h2>
                        ...
                        <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
                            ...
                            {canAccessQA && (
                                <Button
                                    ...
                                    onClick={() => navigate(routes.qa.replace(":eventSessionId", eventSession.id))}
                                >
                            ...
                            {isBuyer && booking.type != BookingType.INVITE && (
                                <>
                                    <Button
                                        variant="outline"
                                    ...
                                    >
                                        ...
                                        <span className="hidden sm:inline">Chi tiết đơn hàng #{booking.id}</span>
                                    </Button>
                ...
                {isEventExpired && (
                    <ReviewSection
                        ...
                        attendeeId={attendeeIdForReview}
                        canReview={canReview}
