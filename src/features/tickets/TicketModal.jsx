...
const getTicketSchema = (eventStart, eventEnd) => {
    return z.object({
        ...
        maximumPerUser: z.preprocess(
            (val) => (val === null || val === "" ? null : Number(val)),
            z.union([z.number().min(1, { message: "Tối thiểu 1 vé/người" }), z.null()])
        ),
        ...
        .refine((data) => {
            if (data.maximumPerUser === null) return true;
            return data.maximumPerPurchase <= data.maximumPerUser;
        }, {
            message: "Max/Đơn không được vượt quá Max/Người",
            path: ["maximumPerPurchase"],
        })
        .refine((data) => {
            if (data.maximumPerUser === null) return true;
            return data.maximumPerUser <= data.quantity;
        }, {
            message: "Max/Người không vượt quá tổng số lượng",
            path: ["maximumPerUser"],
        });
};
        ...
        if (isOpen) {
            if (ticketToEdit) {
                ...
                ticketForm.reset({
                    ...
                    maximumPerUser: ticketToEdit.maximumPerUser || ticketToEdit.maximumPerPurchase || 5,
                ...
                ticketForm.reset({
                    name: '', price: 0, quantity: 100, maximumPerPurchase: 5,
                    maximumPerUser: 5,
                    invitationQuota: 0,
                    openAt: new Date(),
                        ...
                        <DialogHeader className="p-4 border-b bg-gray-50/50">
                            <div className="flex flex-col gap-1 text-left">
                                ...
                                <DialogDescription className="text-gray-500 text-sm">
                                    {isEditing ? "Chỉnh sửa thông tin, giá và số lượng vé." : "Thiết lập vé cho phiên này."}
                                </DialogDescription>
                            </div>
                        </DialogHeader>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5">
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            <FormLabel className="font-bold text-gray-700">
                                                Max/Người <span className="text-red-500">*</span>
                                            </FormLabel>
                                            <FormControl>
                                                <Input
                                                    {...field}
                                                    ...
                                                    type="number"
                                                    className="h-11 disabled:opacity-50 disabled:bg-slate-100"
                                                    min={1}
                                            ...
                                            <div className="flex items-center space-x-2 pt-1">
                                                <Checkbox
                                                    ...
                                                    onCheckedChange={(checked) => {
                                                        if (checked) {
                                                            ticketForm.setValue("maximumPerUser", null, { shouldValidate: true });
                                                        } else {
                                                            const maxPerPurchase = ticketForm.getValues("maximumPerPurchase");
                                                            ticketForm.setValue("maximumPerUser", maxPerPurchase || 1, { shouldValidate: true });
                                                        }
                                                    }}
                                                />
                                                <label
                                                    htmlFor="unlimitedUser"
                                                    className="text-xs font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 text-gray-600 cursor-pointer"
                                                >
                                                    Không giới hạn
                                                </label>
                                            </div>
