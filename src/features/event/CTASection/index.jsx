...
const CTASection = () => {
    ...
    const linkOnlick = user ? routes.organizerRegistration : routes.signin
    return (
        <>
            {
                (user && user.role.name != RoleName.ORGANIZER.key || !user) &&
                <section className="relative mx-auto my-8 max-w-7xl px-4 sm:px-6 lg:px-8">
                    <div className="relative overflow-hidden rounded-2xl bg-brand px-6 py-12 text-center shadow-lg sm:px-12 lg:px-16">
                        <div className="relative z-10 flex flex-col items-center gap-6">
                         ...
                         bg-white px-8 py-3 text-sm font-bold text-brand shadow-sm
                          hover:bg-gray-50 transition-colors"
                                    onClick={() => navigate(linkOnlick)}
                                >
